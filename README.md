...
- **Query execution & results inspection** with the [YASGUI](https://github.com/rdfjs/Yasgui) YASR component, against remote endpoints, or a local embedded oxigraph triplestore
...
Completion and hover requires a target endpoint URL (a `Backend` for Qlue-ls). It can be done in different ways:
...
You can add RDF files (e.g. `.ttl`, `.trig`, `.jsonld`, etc) to a local embedded oxigraph SPARQL endpoint by right clicking the RDF file in the explorer. Then execute the queries against this embedded endpoint using the `local://sparql-endpoint` URL
